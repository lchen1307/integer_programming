...
from gurobipy.gurobipy import GRB, GurobiError
...
try:
    ...
    for i in range(nwidth):
        ...
        rmp_con.append(rmp.addConstr(quicksum(rmp_var[j] * row_coeff[j] for j in range(nwidth)) \
                                     >= amount[i], 'rmpcon_' + str(i)))
        row_coeff[i] = 0
    ...
    sub_var = []
    for i in range(nwidth):
        sub_var.append(sub.addVar(0.0, GRB.INFINITY, 1.0, GRB.CONTINUOUS,'sub_' + str(i)))
